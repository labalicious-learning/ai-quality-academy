...
Provide a small, intentionally buggy training repository with an `AGENTS.md`, a passing test command, and two issues. One issue should be solvable by reading code; the other should be ambiguous and require a question. Verify every learner can open the repository and has no production credentials.
...
| 40–50 | Guided practice: each pair fills in an AI task brief for a broken login button. |
